...
calculateUserPosVel = (time, geodetic, obs_pos, obs_vel) =>
{
    ...
    geodetic.theta      = PredictMath.fMod2p(Time.thetaGJD(time) + geodetic.lon);  /* LMST */
...
calculateLatLonAlt = (_time, pos, geodetic) =>
{
    ...
    geodetic.lon    = PredictMath.fMod2p(geodetic.theta - Time.thetaGJD(_time));   /*radians*/
...
calculateObs = (_time, pos, vel, geodetic, obs_set) =>
{
    ...
    calculateUserPosVel(_time, geodetic, obs_pos, obs_vel);
