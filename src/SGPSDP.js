...
class PredictSGPSDP
{
    ...
    ORBIT_TYPE_LEO          = 1;            /* Low Earth orbit; up to 1200 km. */
    ORBIT_TYPE_ICO          = 2;            /* Intermediate Circular Orbit; up to 1400 km. */
    ORBIT_TYPE_GEO          = 3;            /* Geostationary. */
    ORBIT_TYPE_GSO          = 4;            /* Geosynchronous. */
    ORBIT_TYPE_MOLNIYA      = 5;
    ORBIT_TYPE_TUNDRA       = 6;
    ORBIT_TYPE_POLAR        = 7;
    ORBIT_TYPE_SUNSYNC      = 8;
    ORBIT_TYPE_DECAYED      = 9;
    ...
    SGP4 = (sat, tsince) =>
    {
        let a1, theta2, eosq, betao, betao2, del1, ao, delo, s4, qoms24, perigee,
            ...
            temp1, temp2, temp3, x1m5th, xhdot1, temp, c1sq, xmdf, omgadf, xnoddf,
            omega, xmp, tsq, xnode, tempa, tempe, templ, delomg, delm, tcube, tfour,
            a, e, xl, beta, xn, axn, xll, aynl, xlt, ayn, capu, i = 0, sinepw, cosepw,
            temp4, temp5, temp6, epw, ecose, esine, elsq, pl, r, rdot, rfdot, betal,
            cosu, sinu, u, sin2u, cos2u, rk, uk, xnodek, xinck, rdotk, rfdotk,
            sinuk, cosuk, sinik, cosik, sinnok, cosnok, xmx, xmy, ux, uy, uz, vx,
            vy, vz;

        /* Initialization */
        if (~sat.flags & this.SGP4_INITIALIZED_FLAG) {
            sat.flags |= this.SGP4_INITIALIZED_FLAG;

            /* Recover original mean motion (xnodp) and   */
            /* semimajor axis (aodp) from input elements. */
            a1              = Math.pow(Constants.xke / sat.tle.xno, Constants.tothrd);
            ...
            del1            = 1.5 * Constants.ck2 * sat.sgps.x3thm1 / (a1 * a1 * betao * betao2);
            ao              = a1 * (1 - del1 * (0.5 * Constants.tothrd + del1 * (1 + 134.0 / 81.0 * del1)));
            delo            = 1.5 * Constants.ck2 * sat.sgps.x3thm1 / (ao * ao * betao * betao2);
            ...
            if ((sat.sgps.aodp * (1.0 - sat.tle.eo) / Constants.ae) < (220.0 / Constants.xkmper + Constants.ae)) {
                sat.flags |= this.SIMPLE_FLAG;
            } else {
                sat.flags &= ~this.SIMPLE_FLAG;
            }
            ...
            s4 = Constants.__s__;
            qoms24 = Constants.qoms2t;
            perigee = (sat.sgps.aodp * (1 - sat.tle.eo) - Constants.ae) * Constants.xkmper;
            if (perigee < 156.0) {
                if (perigee <= 98.0) {
                    s4 = 20.0;
                } else {
                ...
                qoms24 = Math.pow((120.0 - s4) * Constants.ae / Constants.xkmper, 4);
                s4 = s4 / Constants.xkmper + Constants.ae;
            } /* FIXME FIXME: End of if(perigee <= 98) NO WAY!!!! */
            ...
            c2 = coef1 * sat.sgps.xnodp * (sat.sgps.aodp *
                            ...
                            0.75 * Constants.ck2 * tsi / psisq * sat.sgps.x3thm1 *
                            (8.0 + 3.0 * etasq * (8 + etasq)));
            sat.sgps.c1 = c2 * sat.tle.bstar;
            sat.sgps.sinio = Math.sin(sat.tle.xincl);
            a3ovk2 = -Constants.xj3 / Constants.ck2 * Math.pow(Constants.ae, 3);
            c3 = coef * tsi * a3ovk2 * sat.sgps.xnodp * Constants.ae * sat.sgps.sinio / sat.tle.eo;
            ...
            sat.sgps.c4 = 2.0 * sat.sgps.xnodp * coef1 * sat.sgps.aodp * betao2 *
                ...
                2.0 * Constants.ck2 * tsi / (sat.sgps.aodp * psisq) *
            ...
            temp1 = 3.0 * Constants.ck2 * pinvsq * sat.sgps.xnodp;
            temp2 = temp1 * Constants.ck2 * pinvsq;
            temp3 = 1.25 * Constants.ck4 * pinvsq * pinvsq * sat.sgps.xnodp;
            ...
            sat.sgps.xmcof = -Constants.tothrd * coef * sat.tle.bstar * Constants.ae / eeta;
            ...
            sat.sgps.delmo = Math.pow(1.0 + sat.sgps.eta * Math.cos(sat.tle.xmo), 3);
            sat.sgps.sinmo = Math.sin(sat.tle.xmo);
            sat.sgps.x7thm1 = 7.0 * theta2 - 1.0;
            if (~sat.flags & this.SIMPLE_FLAG) {
        ...
        if (~sat.flags & this.SIMPLE_FLAG) {
            ...
            delm = sat.sgps.xmcof * (Math.pow(1 + sat.sgps.eta * Math.cos(xmdf), 3) - sat.sgps.delmo);
            temp = delomg + delm;
            xmp = xmdf + temp;
            omega = omgadf - temp;
            tcube = tsq * tsince;
            ...
            tempe = tempe + sat.tle.bstar * sat.sgps.c5 * (Math.sin(xmp) - sat.sgps.sinmo);
            templ = templ + sat.sgps.t3cof * tcube + tfour *
                (sat.sgps.t4cof + tsince * sat.sgps.t5cof);
        } /* End of if (isFlagClear(SIMPLE_FLAG)) */

        a = sat.sgps.aodp * Math.pow(tempa, 2);
        e = sat.tle.eo - tempe;
        ...
        do {
            ...
            if (Math.abs(epw - temp2) <= Constants.e6a) {
                break;
            }
            temp2 = epw;
        } while (i++ < 10);
        ...
        sat.phase = xlt - xnode - omgadf + Constants.twopi;
        if (sat.phase < 0) {
     ...
     * time since epoch in minutes, tle is a pointer to a tle_t
     * structure with Keplerian orbital elements and pos and vel are
    ...
    SDP4 = (sat, tsince) =>
    {
        let a1, del1, ao, delo, s4, qoms24, perigee, pinvsq, tsi,
            ...
            temp1, temp2, temp3, x1m5th, xhdot1, xmdf, xnoddf, tsq,
            tempa, tempe, templ, a, xmam, xl, beta, axn, temp, xll,
            aynl, xlt, ayn, capu, i, sinepw, cosepw, temp4, temp5,
            temp6, epw, ecose, esine, elsq, pl, r, rdot, rfdot,
            betal, cosu, sinu, u, sin2u, cos2u, rk, uk, xnodek,
            xinck, rdotk, rfdotk, sinuk, cosuk, sinik, cosik,
            sinnok, cosnok, xmx, xmy, ux, uy, uz, vx, vy, vz;

        /* Initialization */
        if (~sat.flags & this.SDP4_INITIALIZED_FLAG) {

            sat.flags |= this.SDP4_INITIALIZED_FLAG;
            ...
            del1 = 1.5 * Constants.ck2 * sat.sgps.x3thm1 /
                (a1 * a1 * sat.deep_arg.betao * sat.deep_arg.betao2);
            ao = a1 * (1.0 - del1 * (0.5 * Constants.tothrd + del1 * (1.0 + 134.0 / 81.0 * del1)));
            delo = 1.5 * Constants.ck2 * sat.sgps.x3thm1 /
            ...
            s4 = Constants.__s__;
            qoms24 = Constants.qoms2t;
            perigee = (sat.deep_arg.aodp * (1.0 - sat.tle.eo) - Constants.ae) * Constants.xkmper;
            if (perigee < 156.0) {
                if (perigee <= 98.0) {
                    s4 = 20.0;
                } else {
            ...
            c2 = coef1 * sat.deep_arg.xnodp * (sat.deep_arg.aodp *
                (1.0 + 1.5 * etasq + eeta *
                    (4.0 + etasq)) + 0.75 * Constants.ck2 * tsi / psisq *
            ...
            sat.sgps.c4 = 2.0 * sat.deep_arg.xnodp * coef1 *
                ...
                (0.5 + 2.0 * etasq) - 2.0 * Constants.ck2 * tsi /
            ...
            temp1 = 3.0 * Constants.ck2 * pinvsq * sat.deep_arg.xnodp;
            temp2 = temp1 * Constants.ck2 * pinvsq;
            temp3 = 1.25 * Constants.ck4 * pinvsq * pinvsq * sat.deep_arg.xnodp;
        ...
        this.Deep(this.dpsec, sat);

        xmdf = sat.deep_arg.xll;
        a = Math.pow(Constants.xke / sat.deep_arg.xn, Constants.tothrd) * tempa * tempa;
        ...
        /* Phase in rads */
        sat.phase = xlt - sat.deep_arg.xnode - sat.deep_arg.omgadf + Constants.twopi;
        if (sat.phase < 0.0) {
            sat.phase += Constants.twopi;
        }
        sat.phase = PredictMath.fMod2p(sat.phase);
    ...
    Deep = (ientry, sat) =>
    {
        let eq, aqnv, xmao, xpidot, sinq, cosq, day, xnodce, stem,
            ctem, c, gam, zx, zy, zcosg, zsing, zcosi, zsini, zcosh,
            zsinh, cc, zn, ze, zmo, xnoi, a1, a3, a7, a8, a9, a10,
            a2, a4, a5, a6, x1, x2, x3, x4, x5, x6, x7, x8, z31, z32,
            z33, z1, z2, z3, z11, z12, z13, z21, z22, z23, s3, s2, s4,
            s1, s5, s6, s7, se, si, sl, sgh, sh, eoc, g201, g211, g310,
            g322, g410, g422, g520, g533, g521, g532, sini2, f220,
            f221, f321, f322, f441, f442, f522, f523, f542, f543, xno2,
            ainv2, temp1, temp, bfact, g200, g300, f311, f330, delt, ft,
            xndot, xnddt, xomi, x2omi, x2li, xldot, xl, sinis, cosis,
            zm, zf, sinzf, f2, f3, ses, sis, sls, sel, sil, sll, pgh,
            ph, sinok, cosok, alfdp, betdp, dalf, dbet, xls, dls, xnoh;
        ...
        switch (ientry) {
            case this.dpinit : /* Entrance for deep space initialization */
                ...
                /* Do solar terms */
                sat.dps.savtsn = 1e20;
                zcosg = Constants.zcosgs;
                zsing = Constants.zsings;
                zcosi = Constants.zcosis;
                zsini = Constants.zsinis;
                zcosh = cosq;
                zsinh = sinq;
                cc = Constants.c1ss;
                zn = Constants.zns;
                ...
                for(;;) {
                    ...
                    ze = Constants.zel;
                    zmo = sat.dps.zmol;
                    sat.flags |= this.LUNAR_TERMS_DONE_FLAG;
                } /* End of for(;;) */
                ...
                sat.flags &= ~this.RESONANCE_FLAG;
                sat.flags &= ~this.SYNCHRONOUS_FLAG;
                ...
                if (!((sat.dps.xnq < 0.0052359877) && (sat.dps.xnq > 0.0034906585))) {
                    ...
                    * (1 + 2 * sat.deep_arg.cosio - 3 * sat.deep_arg.theta2));
                    ...
                    bfact = sat.deep_arg.xmdot + sat.deep_arg.xnodot +
                        sat.deep_arg.xnodot - Constants.thdt - Constants.thdt;
                    ...
                    sat.dps.del2 = 2 * sat.dps.del1 * f220 * g200 * Constants.q22;
                    sat.dps.del3 = 3 * sat.dps.del1 * f330 * g300 * Constants.q33 * aqnv;
                    sat.dps.del1 = sat.dps.del1 * f311 * g310 * Constants.q31 * aqnv;
                    ...
                    bfact = sat.deep_arg.xmdot + xpidot - Constants.thdt;
            ...
            case this.dpsec: /* Entrance for deep space secular effects */
                ...
                if (sat.deep_arg.xinc < 0) {
                    ...
                    sat.deep_arg.xnode = sat.deep_arg.xnode + Constants.pi;
                    sat.deep_arg.omgadf = sat.deep_arg.omgadf - Constants.pi;
                }

                if(~sat.flags & this.RESONANCE_FLAG) {
                    return;
                }
                ...
                do {
                    if ( (sat.dps.atime === 0) ||
                        ...
                        if (Math.abs(sat.deep_arg.t) >= Math.abs(sat.dps.atime)) {
                            if (sat.deep_arg.t > 0) {
                                delt = sat.dps.stepp;
                            } else {
                                delt = sat.dps.stepn;
                    ...
                    do {
                        if (Math.abs(sat.deep_arg.t - sat.dps.atime) >= sat.dps.stepp) {
                            sat.flags |= this.DO_LOOP_FLAG;
                            sat.flags &= ~this.EPOCH_RESTART_FLAG;
                        } else {
                        ...
                        if (Math.abs(sat.deep_arg.t) < Math.abs(sat.dps.atime)) {
                            ...
                            sat.flags |= (this.DO_LOOP_FLAG | this.EPOCH_RESTART_FLAG);
                        }

                        /* Dot terms calculated */
                        if (sat.flags && this.SYNCHRONOUS_FLAG) {
                            xndot = sat.dps.del1 * Math.sin(sat.dps.xli - sat.dps.fasx2) + sat.dps.del2 * Math.sin(2 * (sat.dps.xli - sat.dps.fasx4))
                            + sat.dps.del3 * Math.sin(3 * (sat.dps.xli - sat.dps.fasx6));
                            xnddt = sat.dps.del1 * Math.cos(sat.dps.xli - sat.dps.fasx2) + 2 * sat.dps.del2 * Math.cos(2 * (sat.dps.xli - sat.dps.fasx4))
                            + 3 * sat.dps.del3 * Math.cos(3 * (sat.dps.xli - sat.dps.fasx6));
                        } else {
                            xomi = sat.dps.omegaq + sat.deep_arg.omgdot * sat.dps.atime;
                            x2omi = xomi + xomi;
                            x2li = sat.dps.xli + sat.dps.xli;
                            xndot = sat.dps.d2201 * Math.sin(x2omi + sat.dps.xli - Constants.g22)
                            + sat.dps.d2211 * Math.sin(sat.dps.xli - Constants.g22)
                                + sat.dps.d3210 * Math.sin(xomi + sat.dps.xli - Constants.g32)
                            + sat.dps.d3222 * Math.sin(-xomi + sat.dps.xli - Constants.g32)
                            + sat.dps.d4410 * Math.sin(x2omi + x2li- Constants.g44)
                                + sat.dps.d4422 * Math.sin(x2li- Constants.g44)
                                + sat.dps.d5220 * Math.sin(xomi + sat.dps.xli- Constants.g52)
                            + sat.dps.d5232 * Math.sin(-xomi + sat.dps.xli- Constants.g52)
                            + sat.dps.d5421 * Math.sin(xomi + x2li - Constants.g54)
                                + sat.dps.d5433 * Math.sin(-xomi + x2li - Constants.g54);
                            xnddt = sat.dps.d2201 * Math.cos(x2omi + sat.dps.xli- Constants.g22)
                            + sat.dps.d2211 * Math.cos(sat.dps.xli - Constants.g22)
                                + sat.dps.d3210 * Math.cos(xomi + sat.dps.xli - Constants.g32)
                            + sat.dps.d3222 * Math.cos(-xomi + sat.dps.xli - Constants.g32)
                            + sat.dps.d5220 * Math.cos(xomi + sat.dps.xli - Constants.g52)
                            + sat.dps.d5232 * Math.cos(-xomi + sat.dps.xli - Constants.g52)
                            + 2 * (sat.dps.d4410 * Math.cos(x2omi + x2li - Constants.g44)
                                + sat.dps.d4422 * Math.cos(x2li - Constants.g44)
                                + sat.dps.d5421 * Math.cos(xomi + x2li - Constants.g54)
                                + sat.dps.d5433 * Math.cos(-xomi + x2li - Constants.g54));
                        } /* End of if (isFlagSet(SYNCHRONOUS_FLAG)) */

                        xldot = sat.dps.xni + sat.dps.xfact;
                        xnddt = xnddt * xldot;

                        if (sat.flags && this.DO_LOOP_FLAG) {
                    ...
                    } while ((sat.flags & this.DO_LOOP_FLAG) &&
                    (~sat.flags & this.EPOCH_RESTART_FLAG));
                }
                while ((sat.flags & this.DO_LOOP_FLAG) && (sat.flags && this.EPOCH_RESTART_FLAG));

                sat.deep_arg.xn = sat.dps.xni + xndot * ft + xnddt * ft * ft * 0.5;
                xl = sat.dps.xli + xldot * ft + xndot * ft * ft * 0.5;
                temp = -sat.deep_arg.xnode + sat.dps.thgr + sat.deep_arg.t * Constants.thdt;
            ...
            case this.dpper: /* Entrance for lunar-solar periodics */
                sinis = Math.sin(sat.deep_arg.xinc);
                cosis = Math.cos(sat.deep_arg.xinc);

                if (Math.abs(sat.dps.savtsn - sat.deep_arg.t) >= 30) {
                    sat.dps.savtsn = sat.deep_arg.t;
                    zm = sat.dps.zmos + Constants.zns * sat.deep_arg.t;
                    zf = zm + 2 * Constants.zes * Math.sin(zm);
                    sinzf = Math.sin(zf);
                    ...
                    zm = sat.dps.zmol + Constants.znl * sat.deep_arg.t;
                    zf = zm + 2 * Constants.zel * Math.sin(zm);
                    sinzf = Math.sin(zf);
                    f2 = 0.5 * sinzf * sinzf - 0.25;
                    f3 = -0.5 * sinzf * Math.cos(zf);
                ...
                if (sat.dps.xqncl >= 0.2) {
                    ...
                    if(Math.abs(xnoh - sat.deep_arg.xnode) > Constants.pi) {
                        if (sat.deep_arg.xnode < xnoh) {
                            sat.deep_arg.xnode += Constants.twopi;
                        } else {
                            sat.deep_arg.xnode -= Constants.twopi;
                        }
                    }
                    ...
                    sat.deep_arg.xll = sat.deep_arg.xll + sat.dps.pl;
                    sat.deep_arg.omgadf = xls - sat.deep_arg.xll - Math.cos(sat.deep_arg.xinc) * sat.deep_arg.xnode;
                } /* End case dpper: */
                return;
